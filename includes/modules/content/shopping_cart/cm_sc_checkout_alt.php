  ...
  class cm_sc_checkout_alt {
    ...
    function execute() {
      ...
      if ($cart->count_contents() > 0) {
        if (!class_exists('payment')) include(DIR_WS_CLASSES . 'payment.php');
        $payment_modules = new payment;
          
        $initialize_checkout_methods = $payment_modules->checkout_initialization_method();
        if (!empty($initialize_checkout_methods)) {	    		
          $sc_alt_checkout = null;
    ...
    function keys() {
      return array('MODULE_CONTENT_SC_CHECKOUT_ALT_STATUS', 'MODULE_CONTENT_SC_CHECKOUT_ALT_CONTENT_WIDTH', 'MODULE_CONTENT_SC_CHECKOUT_ALT_SORT_ORDER');
    }
  }
